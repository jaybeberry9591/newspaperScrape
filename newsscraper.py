...
json.
"""
#to run this project i terminal type "python newsscraper.py NewsPapers.json"
import sys
import json
...
def run(config, limit=4):
    ...
    try:
        with open("scraped_english_articles.json", "w") as outfile:
...
def main():
    ...
    if len(args) < 2:
        sys.exit("Usage: newsscraper.py englishnewspaper.json")
...
# python newsscraper.py englishnewspaper.json